...
uint8_t lll_chan_sel_roundrobin(uint16_t counter, uint16_t chan_id, uint8_t *chan_map,
		    uint8_t chan_count) {
		uint8_t chan_next;
		uint8_t cnt = 0;
		uint8_t chan_pos = 0;
		do {
			chan_next = (counter+cnt+1)%37;
			chan_pos = 1 << (chan_next & 0x07);
			cnt++;
		} while (((chan_map[chan_next>>3]&chan_pos)==0) || cnt > 37);
		return chan_next;
		}
#endif /* CONFIG_BT_CTLR_CHAN_SEL_ROUNDROBIN */
